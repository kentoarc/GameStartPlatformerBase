...
var standardLaserRight				: Transform;
var standardLaserLeft				: Transform;
...
function Update()
{
	...
	if(Input.GetKeyDown("e"))
	{
		if(moveDirection == 1)
		{
			Instantiate(standardLaserRight, projectileSocketRight.position, projectileSocketRight.rotation);
		}
		
		if(moveDirection == 0)
		{
			Instantiate(standardLaserLeft, projectileSocketRight.position, projectileSocketRight.rotation);
		}
	}
	...
	if( controller.isGrounded )											// if player is on the ground
	{
		...
		{
			Log ("Idle Right");									// Animation call to idle right
		}
		if ( velocity.x == 0 && moveDirection == 0)						// Idle left
		{
			Log ("Idle Left");									// Animation call to idle left
		}
	...
	if ( !controller.isGrounded )							// if player is in the air
	{
		...
		if (moveDirection == 0)								// facing left
		{
			if (jumpEnable)									// jump left standard
			{
				...
				Log ("Standard Jumping Left");		// Animation call to standard left jump	
			}
			if (runJumpEnable)								// run left jump 
			{
				velocity.x *= runSpeed;						// run speed * current move speed
				Log("Run Jumping Left");				// Animation call to Run left Jump
			}
			if (crouchJumpEnable)							// jump from left crouch
			{
				velocity.x *= walkSpeed;					// walk speed * current move speed
				Log("Crouch Jumping Left");			// Animation call to left crouch jump
			}
		}
		...
		if (moveDirection == 1)								// facing right
		{
			...
			{
				velocity.x *= walkSpeed;					// walk speed * current move speed
				Log ("Standard Jumping Right");		// Animation call to standard right jump
			}
			if (runJumpEnable)								// run right jump 
			{
				velocity.x *= runSpeed;						// run speed * current move speed
				Log("Run Jumping Right");				//Animation call to Run right Jump
			}
			if (crouchJumpEnable)							// jump from right crouch
			{
				velocity.x *= walkSpeed;					// walk speed * current move speed
				Log("Crouch Jumping Right");			// Animation call to crouch right jump
			}
		}
